...
- **Constraint violations.** Every feeder operates inside three limits:
  ...
  Stated as three real, checkable inequalities:
  $$
  |I_{line}| \le I_{rated}, \qquad V_{min} \le V_{pu} \le V_{max}
  $$
  $$
  \text{VUF} = 100 \times \frac{\max_p |V_p - \bar V|}{\bar V} \le \text{VUF}_{max}
  $$
  where $\bar V = \tfrac{1}{3}\sum_p V_p$ is the average of the three real
  phase voltages and VUF is the NEMA MG1 §14.35 voltage-unbalance factor,
  the same "maximum deviation from the average, as a percentage" definition
  a utility's own compliance report already uses. Fluctuation and load
  growth are what push a feeder toward one of these three limits; the
  limits themselves are what actually gets violated, and a real violation
  is not just a number out of range, it risks equipment damage, real power
  losses, a shortened lifespan for feeder assets, and, in the worst case, an
  outage.
...
**per-unit (pu)**:
$$
V_{pu} = \frac{V_{actual}}{V_{nominal}}
$$
the actual voltage divided by the network's nominal voltage at that
point. A bus sitting exactly at nominal reads 1.0 pu,
...
a set of equations. Concretely, one real equation per bus $i$:
$$
S_i = P_i + jQ_i = V_i \sum_{j} Y_{ij}^{*} V_j^{*}
$$
where $V_i$ is bus $i$'s own complex voltage phasor, $S_i$ is the real and
reactive power injected there (positive for a source, negative for a real
customer `Load`), and $Y_{ij}$ is the admittance between buses $i$ and $j$,
built directly from every line's own real impedance. "Solving" the network
means finding the one set of bus voltages $\{V_i\}$ that satisfies every
one of these equations at once, given each `Load`'s own specified demand,
the same real nonlinear system every power-flow solver, OpenDSS included,
iterates toward rather than solves in closed form.
...
Chapter 3 completes this part's own foundation, recovering which phase a
customer is actually connected to from nothing but voltage patterns, a
real, common gap in utility records.
...
Part 8's own forecasts eventually become the input to exactly the kind of
power-flow check this chapter just ran, testing grid health under
