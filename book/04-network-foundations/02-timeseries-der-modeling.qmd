...
fix could touch. Chapter 3 completes this part's own foundation next, recovering which
phase a customer is actually connected to from nothing but voltage
patterns, a real, common gap in utility records, reusing the same
`ark.dss.Circuit` this chapter just extended.

This network model is what every later part in this book builds on: Part
5 groups customers and feeders by how they actually behave, not just how
they are labeled; Part 6 turns that grouping into a real ranking and
recommendation problem; Part 7 catches a compliance problem, or a meter
reporting bad data, as it happens rather than after a complaint call; and
Part 8's own forecasts eventually become the scenario driver for exactly
the kind of hosting-capacity study this chapter just ran, testing
mitigation strategies like Volt-Watt, Volt-VAr, and storage against
forecasted {{< acr DER >}} growth rather than today's snapshot.
