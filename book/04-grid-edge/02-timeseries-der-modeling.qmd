...
the network. The same real 31-customer AusNet feeder runs across a real
week of demand, then answers two real business questions: how much rooftop
solar, and how much {{< acr EV >}} charging, can this specific feeder
actually take before someone has to act on it. The two answers turn out to
be very different problems, at different times of day, binding on
different constraints, and the fix built for the first one does not
generalize to the second.

## Getting the data

This chapter needs the same real AusNet {{< acr LV >}} network and
smart-meter data Chapter 1 introduced, plus one more real dataset: EV
...
The first downloads the network model and two real datasets, 342
anonymized customers' active-power readings and their matched normalized
{{< acr PV >}} generation, both at 30-minute resolution for a full year.
The second downloads real EV home-charging profiles derived from a UK
metering trial. That second source ships no license, so this chapter uses
it as a reference for building its own scenario, not as data reused
verbatim; every number reported below is this notebook's own.

## This chapter's feeder

Every section below runs on one network: the real 31-customer AusNet
feeder Chapter 1 introduced, the same distribution transformer, the same
24 line segments and 31 single-phase service cables. A result from the
middle of this chapter and a result from the end describe the same
physical feeder, not two unrelated ones.
...
Every one of the feeder's 31 loads gets its own real customer profile,
sampled from the 342 available. Attaching a `LoadShape` is one command,
but trusting it silently is a habit worth breaking: it costs little to
cross-check the real data array, what a live solve reports back, and what
the `LoadShape` object itself has stored, against each other.
...
data. A real time-series study covers more than one day, so every
...
and a temperature profile instead, attached to a real house on this same
feeder rather than a separate demonstration network.
...
This is the real business question a {{< acr DSO >}} faces every time a
new rooftop solar application arrives: does connecting one more customer's
...
upgrade that may not have been necessary; say yes too rarely and a real
feeder starts violating compliance. A hosting-capacity study is how that
...
introduced the three real constraints an {{< acr LV >}} network operates
...
thermal ceiling well before its voltage limit, which is exactly why a real
...
threshold: full output stays available up to 1.05 pu, then ramps linearly
...
option to try on a real feeder before accepting reduced generation.
...
This chapter also draws on real {{< acr EV >}} home-charging profiles
derived from a UK metering trial: real metered charging behavior, not a
synthetic assumption, for Level 1 (3.68kW) and Level 2 (7.36kW) chargers,
diversified across a group of real vehicles to account for the fact that
not everyone charges at the same time. The source repository for this
data ships no license, unlike the network and smart-meter data behind the
rest of this chapter, so it is used here as a reference for building this
...
transformer chart already flagged as the feeder's busiest. The diversified
curve peaks around 1.5kW per {{< acr EV >}}, not the charger's full
7.36kW rating: most vehicles in the group are not charging at any given
moment, so the average demand per {{< acr EV >}} stays well under the
rated maximum. That diversity is a real, physical mitigating effect,
worth testing directly rather than assuming.

Every {{< acr EV >}}-owning house on this feeder gets its own copy of
the real diversified shape, an additional `Load` that adds demand on top
of a house's existing consumption rather than offsetting it. This time the
worst-case day is not the sunniest, it is the day with the highest real
...
is doing real work here: real UK-trial data says not enough
...
at the same real evening peak and charge at the charger's full rated
power, no diversity at all.
...
on the feeder, but an {{< acr EV >}} charger modeled as a `Load` is not a
{{< acr PV >}} inverter, and is not affected by either control at all.
The mitigation that solved the midday violation has nothing to act on in
an evening {{< acr EV >}} scenario. Of the levers this chapter has built,
only `Storage` can help both peaks: charge from midday {{< acr PV >}}
surplus, discharge into the evening {{< acr EV >}}-and-dinner peak, the
two real problems this feeder's own daily cycle actually produces. That
is precisely the kind of {{< acr DER >}}-aware mitigation strategy Part
4's fourth thread evaluates in full.

## Do it yourself

This chapter's hosting-capacity study answered one question: on this
feeder's single sunniest day of the year, how much {{< acr PV >}} is too
much. A related but different question is worth working through before
...
the tallest curve in this chapter's own {{< acr PV >}} diversity chart. A
...
holding three candidate daily profiles for that season, real data from
the same AusNet dataset, paired with a specific real demand day per
season.
...
representative candidates per season. A real hosting-capacity study
states which one it used, the same way this chapter just did twice, with
two different, both defensible, answers.
...
diversity, a real week of demand, a real violation at 80% {{< acr PV >}}
penetration, and two real fixes for it, then a second, genuinely
different violation from {{< acr EV >}} charging that neither fix could
touch. Part 4 now follows four threads, each building on the same
`ark.dss.Circuit` this chapter just extended:
