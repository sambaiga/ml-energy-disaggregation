...
## What a low-voltage network actually is
...
```{python}
#| label: fig-grid-centralized-decentralized
#| fig-cap: "The traditional grid moves power one way, plant to customer. A DER-rich low-voltage network moves it both ways."
from ark.plot.diagrams import centralized_vs_decentralized_grid_diagram

centralized_vs_decentralized_grid_diagram()
```

A real low-voltage feeder branches the way a tree does: one three-phase
backbone leaves the transformer, then splits into single-phase taps,
called service drops, each reaching one customer. @fig-lvcircuit-topology
shows a real one, an anonymized residential circuit with 31 customers
spread across three phases. The data comes from [AusNet
Services](https://www.ausnetservices.com.au/), an Australian distribution
utility, via [Team-Nando's DER hosting-capacity tutorial
series](https://github.com/Team-Nando). The network this chapter builds has
exactly the same shape, a transformer, a backbone, single-phase taps, just
three customers instead of 31. Small enough to read every line of its
definition, large enough that everything it teaches scales up unchanged.

![A real low-voltage feeder: 31 single-phase customers on three phases, fed from one distribution transformer. Source: Team-Nando's DER hosting-capacity tutorial series, built on AusNet Services data.](LVcircuit-topology.png){#fig-lvcircuit-topology}

## Why the low-voltage network matters now

Clean-energy investment reached \$1.7 trillion globally in 2023
[@iea2023investment], and a growing share of it is landing somewhere new.
For decades, power flowed one way through the chain above: plant,
transmission, substation, feeder, meter. Consumers only ever received.

Rooftop {{< acr PV >}}, home batteries, and {{< acr EV >}} charging break
that one-way assumption, and almost all of them connect at the low-voltage
level, not the transmission or medium-voltage tiers above it
[@bystrom2022distribution]. A household with solar panels is not a passive
consumer anymore; on a sunny afternoon it can be exporting power back
through the exact feeder that used to only deliver it. The low-voltage
network was never designed to carry power in both directions, and it is
where the energy transition is actually showing up, meter by meter.

## From smart meter to network model

A smart meter and a network model answer two different questions, and
mixing them up is the mistake this whole chapter exists to prevent. A meter
answers "how much power is this customer using right now," a single Load's
kW and kvar, reported once per interval. A network model answers "what does
that demand do to the rest of the feeder," a question no single meter, or
even every meter on the street added together, can answer on its own,
because it depends on where each customer sits relative to the others and
how the wire between them behaves.

The bridge between the two is direct: a meter's own reading becomes a
network model's input. One customer's demand becomes one `Load`'s `kW`; a
whole time series of meter readings becomes what OpenDSS calls a
`LoadShape`, the mechanism Chapter 2 picks up directly. Combine that meter
data with the network's topology, the impedance of every line, the phase
each customer connects to. None of that comes from any meter. Solving the
network with both answers the harder question: not just what one customer
is doing, but what it means for the voltage one pole over, or the
transformer three poles up the street. That combination, real meter data
plus real topology, is the mechanism behind every thread the rest of
Part 4 follows.

## Three ways {{< acr DER >}} strain a feeder

Distributed energy resources do not strain a low-voltage network in one
way. They strain it in three, and each has a distinct signature:
...
- **Fluctuation.** Solar output rises and falls with the sun, not with
  ...
  to compensate for that swing every single day.
- **Load growth.** Unmanaged {{< acr EV >}} charging can push a feeder's
  ...
  out the peak instead of just raising it.
- **Constraint violations.** Every feeder operates inside three limits:
  ...
  three limits; the limits themselves are what actually gets violated.
...
phase, the same shape as @fig-lvcircuit-topology above, just smaller. The
full sequence of commands that builds it, transformer, linecodes, lines,
loads, one line at a time, is in the companion notebook; here is the shape
of it:
...
a set of equations. From there, the per-unit voltage at every bus is a
DataFrame away:

{{< embed 01-opendss-lv-modeling-code.ipynb#b517c96d >}}

Every house here sits comfortably inside the 0.94-1.10 pu band from the
concept box above. Reading a table of numbers is one way to check that;
seeing the network is another, and it is usually the faster one. The same
solved circuit, drawn as a diagram, colored by exactly that compliance
band:
...
A solved circuit holds more than bus voltages. `Circuit` turns every
...
as a query, the same escape hatch OpenDSS scripting always offers:

{{< embed 01-opendss-lv-modeling-code.ipynb#37141e33 >}}

Iteration and results combine naturally, no different from looping over
any other Python collection:

{{< embed 01-opendss-lv-modeling-code.ipynb#7b4d8ac2 >}}

The same pattern that produced a bus-voltage table produces a power table
for any element type, one row per phase, not a flat list to index into by
hand:
...
## Checking a feeder across a full day
...
{{< embed 01-opendss-lv-modeling-code.ipynb#47486a5b >}}
