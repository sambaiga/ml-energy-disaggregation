...
![An illustrative eigenvalue spectrum, one dominant direction followed by a long, smoothly decaying tail. The 90%-variance rule and profile likelihood's own change point land at genuinely different points along the same curve, since a smoothly decaying tail gives neither method an obvious place to stop by construction.](../../assets/component-selection.svg){#fig-component-selection}
...
![Two illustrative populations, both reading as well separated by every formula above. Left: a real, even two-way split. Right: one dominant group plus two isolated points, a genuine outlier rather than a real archetype. Neither silhouette, Davies-Bouldin, nor Calinski-Harabasz has a term for population share, so both score as a win.](../../assets/separation-index-blind-spot.svg){#fig-separation-index-blind-spot}
...
![Two ways to ask whether a clustering survives data coming out slightly differently. Prediction strength splits the data once and checks whether one half's own centroids still predict the other half's own pairs correctly. Cluster-wise stability bootstraps the whole dataset and checks how well one specific cluster survives being found again.](../../assets/resampling-mechanisms.svg){#fig-resampling-mechanisms}
