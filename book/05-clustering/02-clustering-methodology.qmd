...
Chapter 1 already gave silhouette, Davies-Bouldin, and Calinski-Harabasz their own formal definitions [@rousseeuw1987silhouette; @daviesbouldin1979clusterseparation; @calinskiharabasz1974dendrite]. All three share the same structural blind spot: each is a function of distances or variances alone, with no term that penalises one group holding almost none of the population.
...
Chapter 1 already gave both resampling-based methods that answer this second question their own formal definitions: Tibshirani and Walther's prediction strength [@tibshiranwalther2005predictionstrength], and Hennig's cluster-wise stability just below. Neither, as far as this book's own literature search found, has been applied to smart-meter customer archetyping before. As a brief reminder, prediction strength splits the data into two random halves $A$ and $B$, fits centroids on $A$'s own clustering, and checks whether those centroids still predict $B$'s own same-cluster pairs correctly.
$$
\text{ps}(C) = \frac{1}{n_C(n_C - 1)} \sum_{\substack{i, i' \in C \\ i \neq i'}} \mathbb{1}\!\left[\text{pred}(i) = \text{pred}(i')\right]
$$
Prediction strength for $K$ is the minimum of $\text{ps}(C)$ over $B$'s own clusters, and the recommendation is to pick the largest $K$ scoring above 0.8 to 0.9. A genuine limitation is worth stating plainly rather than glossed over: a cluster with fewer than two members contributes no pairs, so a singleton is excluded from its own split's minimum rather than penalised by it. Prediction strength validates whether $K$ itself is well supported globally; it is not, by itself, guaranteed to flag one small outlier cluster hiding inside an otherwise well-supported partition.
...
This is the gap Hennig's cluster-wise stability closes [@hennig2007clusterwise], bootstrap-resampling the data, refitting centroids, and checking the Jaccard overlap between each original cluster and its best match among the resample's own predicted clusters.
$$
